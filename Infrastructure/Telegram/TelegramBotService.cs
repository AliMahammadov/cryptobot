...
namespace CryptoSense.Infrastructure.Telegram
{
    public class TelegramBotService : BackgroundService, ITelegramBotService
    {
        ...
        private async Task HandleIncomingMessageAsync(string chatId, string username, long? userId, long messageId, string text)
        {
            ...
            // =========================================================================
            // 1. EXPLICIT LOGIN ATTEMPTS (E.G. "Ali 23031999Am", "Murad 123456", "/login ...")
            // =========================================================================
            ...
            // Only evaluate credentials if user is NOT logged in OR explicitly types /login /admin
            ...
            if ((!isAlreadyLoggedIn || isExplicitLoginCommand) && !hasActiveState && !isMenuButtonClick && (loginParts.Length >= 2 || text.Contains("23031999Am")))
            {
                ...
                if (isValid && user != null)
                {
                    ...
                    if (user.Role == UserRole.Admin)
                    {
                        ...
                        await SendMessageAsync(welcomeAdmin, chatId, TelegramKeyboards.BuildAdminKeyboard());
                        return;
                    }
                    // REGULAR USER LOGIN
                    else
                    {
                        ...
                        await SendMessageAsync(onboardingMsg, chatId, TelegramKeyboards.BuildUserKeyboard(settings));
            ...
            // =========================================================================
            // 3. UNAUTHENTICATED USERS PROMPT
            // =========================================================================
            ...
            // =========================================================================
            // 4. AUTHENTICATED REGULAR USERS FLOW
            // =========================================================================
            var userSettings = GetSettings(chatId);

            // Check if user was deleted
            var allUsers = await userManager.GetAllUsersAsync();
            var isStillRegistered = allUsers.Any(u => u.Username.Equals(userSettings.Username, StringComparison.OrdinalIgnoreCase));
            if (!isStillRegistered)
            {
                await RevokeUserSessionAsync(userSettings.Username);
                return;
            }
            ...
            if (_userStates.TryGetValue(chatId, out var coinDelState) && coinDelState == "USER_WAITING_DELETE_COIN")
            {
                ...
                if (userSettings.Coins.Contains(coinToDel))
                {
                    ...
                    await SendMessageAsync(succMsg, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                }
                else
                {
                    var cleanDel = coinToDel.Replace("USDT", "");
                    await SendMessageAsync($"⚠️ <b>'{cleanDel}' coini siyahınızda tapılmadı!</b>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                }
                return;
            }
            ...
            if (_userStates.TryGetValue(chatId, out var coinAddState) && coinAddState == "WAITING_COIN_INPUT")
            {
                ...
                if (parts.Length > 0 && !text.StartsWith("/"))
                {
                    ...
                    var conf = $"✅ <b>Seçilmiş coinləriniz yeniləndi ({userSettings.Coins.Count}/10 ədəd):</b>\n\n" +
                               $"<code>{cleanList}</code>";
                    await SendMessageAsync(conf, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                    return;
                }
            }
                ...
                text == "🌟 Bütün Zamanlar (Hamısı) Siqnalları")
            {
                ...
                if (text.Contains("Bütün Zamanlar") || text.Contains("Hamısı"))
                {
                    ...
                    userSettings.LastResumeTime = DateTime.UtcNow;
                    await SendMessageAsync("⏳ <b>Bütün zaman çərçivələri (3m, 5m, 15m, 1h, 4h) üzrə bazar skan edilir...</b>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                ...
                else
                {
                    ...
                    await SendMessageAsync($"⏳ <b>Yalnız ({targetTf}) üzrə 78%+ Confluence siqnalları axtarılır... (Profiliniz '{targetTf}' olaraq təyin edildi)</b>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                    ...
                    if (found == 0)
                    {
                        await SendMessageAsync($"ℹ️ Hal-hazırda {targetTf} zamanında tələblərə cavab verən aktiv siqnal yoxdur.", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                    }
                    return;
                }
            }
            ...
            if (text == "⏱ 3 Dəqiqə (3m)" || text == "3m")
            {
                ...
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>3m</code>\n<i>Artıq yalnız 3m siqnalları alacaqsınız.</i>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text == "⏱ 5 Dəqiqə (5m)" || text == "5m")
            {
                ...
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>5m</code>\n<i>Artıq yalnız 5m siqnalları alacaqsınız.</i>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text == "⏱ 15 Dəqiqə (15m)" || text == "15m")
            {
                ...
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>15m</code>\n<i>Artıq yalnız 15m siqnalları alacaqsınız.</i>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text == "⏱ 1 Saat (1h)" || text == "1h")
            {
                ...
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>1h</code>\n<i>Artıq yalnız 1h siqnalları alacaqsınız.</i>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text == "⏱ 4 Saat (4h)" || text == "4h")
            {
                ...
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>4h</code>\n<i>Artıq yalnız 4h siqnalları alacaqsınız.</i>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text == "🌟 Bütün Zamanlar (Hamısı)" || text == "Hamisi" || text == "Hamısı")
            {
                userSettings.Timeframe = "Hamısı";
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync($"✅ <b>Zaman Çərçivəsi Təyin Edildi:</b> <code>Hamısı</code>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            else if (text.Contains("Geri") || text.Contains("Əsas Menyu") || text == "/menu")
            {
                await SendMessageAsync("📊 <b>Əsas Menyu:</b>", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                return;
            }
            ...
            if (text == "/start" || text == "/help" || text.Contains("Menyu"))
            {
                ...
                await SendMessageAsync(welcome, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            else if (text.Contains("Dayandır") || text.Contains("Dayandir") || text == "/stop")
            {
                userSettings.IsActive = false;
                await SendMessageAsync("🛑 <b>Bildirişlər və nəticə hesabatları dayandırıldı.</b>\n\nArxa fondan sizə heç bir siqnal və nəticə göndərilməyəcək.\nYenidən başlamaq üçün 'Bildirişləri Başlat' düyməsinə vurun.", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            else if (text.Contains("Başlat") || text.Contains("Baslat") || text == "/resume")
            {
                userSettings.IsActive = true;
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync("▶️ <b>Bildirişlər aktivləşdirildi!</b>\n\nSeçdiyiniz zaman çərçivəsi (" + userSettings.Timeframe + ") üzrə 24/7 siqnallar və nəticələr göndəriləcək.", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            else if (text.Contains("Sıfırla") || text.Contains("Sifirla") || text == "/clear" || text == "/reset")
            {
                userSettings.LastResumeTime = DateTime.UtcNow;
                await SendMessageAsync("🧹 <b>Keçmiş siqnal izləmələri profiliniz üçün sıfırlandı!</b>\n\nBundan əvvəlki köhnə siqnalların nəticəsi sizə gəlməyəcək. Yalnız bu andan etibarən yaranan təzə siqnallar izlənəcək.", chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            else if (text.Contains("Statistika") || text == "/stats")
            {
                ...
                await SendMessageAsync(msg, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            ...
            else if (text.Contains("Coinlərim") || text.Contains("Coinlerim") || text == "/my")
            {
                if (userSettings.Coins.Count == 0)
                {
                    ...
                    await SendMessageAsync(emptyMsg, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                    return;
                }
                ...
                await SendMessageAsync(sb.ToString(), chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            ...
            else if (text.Contains("Coin Sil") || text == "/delcoin")
            {
                ...
                await SendMessageAsync(prompt, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            ...
            else if (text.Contains("Bitcoin") || text == "/btc")
            {
                ...
                await SendMessageAsync(btcMsg, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    await SendMessageAsync(helpMsg, chatId, TelegramKeyboards.BuildUserKeyboard(userSettings));
                }
            }
        }

        private async Task HandleSuperAdminFlowAsync(string chatId, string text, IUserManagerService userManager)
        {
            ...
            if (_userStates.TryGetValue(chatId, out var state) && state == "ADMIN_WAITING_CREATE_USER")
            {
                ...
                {
                    await SendMessageAsync("⚠️ <b>Formatı düzgün daxil edin!</b>\nMəsələn: <code>Murad 123456</code>", chatId, TelegramKeyboards.BuildAdminKeyboard());
                    return;
                }
            }
            ...
            if (_userStates.TryGetValue(chatId, out var pwdState) && pwdState == "ADMIN_WAITING_RESET_PWD")
            {
                ...
                {
                    await SendMessageAsync("⚠️ <b>Formatı düzgün daxil edin!</b>\nMəsələn: <code>Murad yeni123</code>", chatId, TelegramKeyboards.BuildAdminKeyboard());
                    return;
                }
            }
